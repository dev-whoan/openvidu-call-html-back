...
app.get('/', async( req: Request, res: Response) => {
	try{
		...
		let session: Session = openviduService.searchSessionBySessionId(sessionId);
		let response = { code: 200, sessionInfo: circularStringify(session) };
...
app.post('/', async (req: Request, res: Response) => {
	try {
		...
		let date = null;
        	let sessionCreated = await openviduService.createSession(sessionId);
