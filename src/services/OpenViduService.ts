import { Connection, ConnectionProperties, OpenVidu, OpenViduRole, Recording, Session, SessionProperties, RecordingMode } from 'openvidu-node-client';
import { OPENVIDU_URL, OPENVIDU_SECRET, CALL_AUTO_RECORDING } from '../config';
...
export class OpenViduService {
	...
	public async createSession(sessionId: string): Promise<Session> {
		...
		console.log(`auto recording? ${CALL_AUTO_RECORDING}, type? ${typeof CALL_AUTO_RECORDING}`);
//		let sessionProperties: SessionProperties = { customSessionId: sessionId, recordingMode: RecordingMode.ALWAYS };
		let sessionProperties: SessionProperties = { customSessionId: sessionId };
		if(CALL_AUTO_RECORDING === 'true'){
			sessionProperties.recordingMode = RecordingMode.ALWAYS;
		}
	...
	public searchSessionBySessionId(sessionId: string): Session {
